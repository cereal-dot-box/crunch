...
export function Skeleton({ variant = 'card', className = '' }: SkeletonProps) {
  ...
  if (variant === 'card') {
    return (
      <div className={`${baseClasses} p-4 sm:p-6 space-y-4 ${className}`}>
  ...
  if (variant === 'list-item') {
    return (
      <div className={`${baseClasses} p-4 sm:p-6 min-h-[72px] flex items-center justify-between ${className}`}>
  ...
  if (variant === 'stat-card') {
    return (
      <div className={`${baseClasses} p-4 sm:p-6 space-y-2 ${className}`}>
        <div className="h-3 bg-gray-300 rounded w-1/2"></div>
